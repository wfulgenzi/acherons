    ...
    <aside className="w-64 shrink-0 min-h-screen bg-brand-100 border-r border-brand-200 flex flex-col">
      ...
      <div className="px-6 pt-8 pb-6 border-b border-brand-200">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-brand-500 text-xs mt-0.5 font-medium tracking-wide uppercase">Admin panel</p>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 px-3 py-5 space-y-1">
        {NAV_ITEMS.map((item) => (
          ...
          >
            <span className={isActive(item.href, item.exact) ? "text-white/80" : "text-brand-500"}>
              {item.icon}
            </span>
            {item.label}
          </Link>
